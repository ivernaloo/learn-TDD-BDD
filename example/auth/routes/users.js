...
const usersModel = require(process.env.USERS_MODEL ? path.join('..', process.env.USERS_MODEL) : '../models/users-rest'); // import data crud mainpulate

exports.initPassport = function(app) {
  app.use(passport.initialize());
  app.use(passport.session()); // session lifecycle
};

exports.ensureAuthenticated = function(req, res, next) {
  // req.user is set by Passport in the deserialize function
  if (req.user) next(); // detect user wether be detected
                        // just detect the auth info
...
router.get('/auth/twitter/callback',
  passport.authenticate('twitter', { successRedirect: '/',
                                     failureRedirect: '/users/login' })); // twitter login middleware
...
passport.use(new TwitterStrategy({
    consumerKey: "V5oBDLJOGsC7QztZlRqAk8sI4",   // just public to user ?
    consumerSecret: "0Vc1HHUEY3a4YYYfNJDFHPOiljruKMTiecHYyAufxN2Mc0Gxbm", // open to all people
