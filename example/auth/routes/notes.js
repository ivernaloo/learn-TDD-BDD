...
// Add Note. (create), and middleware ,
// express router can and router in the handler
router.get('/add', usersRouter.ensureAuthenticated, (req, res, next) => {
    var user = req.user ? req.user : undefined;
    res.render('noteedit', {    // render the template
        title: "Add a Note",
        docreate: true,
        notekey: "",
        note: undefined,
...
router.post('/save', usersRouter.ensureAuthenticated, (req, res, next) => {
    var p;
    if (req.body.docreate === "create") {   // create logic
        p = notes.create(req.body.notekey,
                req.body.title, req.body.body);
    } else {
        p = notes.update(req.body.notekey,  // notekey
                req.body.title, req.body.body);
    }
    p.then(note => {    // notekey
