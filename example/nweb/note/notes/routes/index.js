...
var getKeyTitlesList = function() {
    return notes.keylist() // query from db
    .then(keylist => {
        ...
        return Promise.all(keyPromises); // combine all promise
    });
};
...
module.exports.socketio = function(io) {
    ...
    notes.events.on('notecreated', emitNoteTitles); // broadcast all data to global
